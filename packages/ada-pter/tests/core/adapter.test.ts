...
describe("context fields", () => {
	...
	test("defaults Content-Type to application/json when missing", async () => {
		let capturedHeaders: any;

		const handler: ApiHandler = {
			getRequestConfig: (_ctx) => ({
				url: "https://api.test.com/v1/completions",
				method: "POST",
				headers: {},
				body: "{}",
	...
	test("auto JSON.stringify when Content-Type is application/json and body is not string", async () => {
		let fetchedInit: any;
		mockFetch.mockImplementation((_url: any, init: any) => {
			fetchedInit = init;
		...
		expect(fetchedInit.body).toBe(JSON.stringify({ hello: "world" }));
	});
