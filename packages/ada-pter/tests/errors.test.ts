...
describe("NoProviderError", () => {
  const err = new NoProviderError("gpt-4");

  test("stores model field", () => {
    expect(err.model).toBe("gpt-4");
  });

  test("formats message with guidance", () => {
    expect(err.message).toContain('No provider found for model "gpt-4"');
  });

  test("sets name to NoProviderError", () => {
    expect(err.name).toBe("NoProviderError");
  });

  test("instanceof chain: Error -> AdaPterError -> NoProviderError", () => {
    expect(err).toBeInstanceOf(Error);
    expect(err).toBeInstanceOf(AdaPterError);
  ...
  test("handles model with prefix", () => {
    const prefixed = new NoProviderError("openai/gpt-4");
    expect(prefixed.model).toBe("openai/gpt-4");
    expect(prefixed.message).toContain('"openai/gpt-4"');
  });
});
...
describe("error discrimination via instanceof in try/catch", () => {
  ...
  test("can distinguish all error subclasses from each other", () => {
    ...
    expect(noProviderErrors).toHaveLength(1);
    expect(adapterErrors).toHaveLength(4); // all four are AdaPterError
  });
});
